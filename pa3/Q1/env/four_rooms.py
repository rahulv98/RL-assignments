...
class FourRooms(gym.Env):
	...
	def __init__(self):
		...
		self.room_connections = [[103, 25], [25, 56], [56, 77], [77, 103]]
	...
	def set_goal(self, goal='G1'):
		if goal == 'G1':
			self.goal = [1, [6, 2]]
			self.goal_global = [7, 9]
		
		elif goal == 'G2':
			self.goal = [2, [1, 2]]
			self.goal_global = [9, 9]
	...
	def get_global_coord(self, curr_state):
		room, coord_local = self.decode(curr_state)
		if room == 0:
			return [sum(i) for i in zip(coord_local, [1, 1])]

		elif room == 1:
			return [sum(i) for i in zip(coord_local, [1, 7])]

		elif room == 2:
			return [sum(i) for i in zip(coord_local, [8, 7])]

		elif room == 3:
			return [sum(i) for i in zip(coord_local, [7, 1])] 

	def render(self, mode='human', close=False):
		img = np.loadtxt(os.path.abspath(os.getcwd()) + '/env/room.txt', delimiter=',')		
		fig = plt.figure(1, figsize=(6,6))
		r, c= self.get_global_coord(self.state)
		img[r, c] = 0.7
		img[self.goal_global[0], self.goal_global[1]] = 0.3
		plt.clf()
		plt.pcolor(img[::-1], cmap='terrain', edgecolor='k')
		fig.canvas.draw()
		# plt.show()	
		plt.pause(0.0001)
		return
